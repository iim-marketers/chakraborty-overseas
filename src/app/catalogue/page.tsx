...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    description: "Range sheets with grades, standards, sizes and finishes for every fastener range we export.",
    url: "/catalogue",
  },
};
...
function fileSize(file: string) {
  try {
    const bytes = statSync(path.join(process.cwd(), "public", "catalogue", file)).size;
...
export default function CataloguePage() {
  return (
    <>
        ...
        <div className="shell">
          ...
          <Reveal>
            <div className="grid gap-8 overflow-hidden rounded-[var(--radius-tile)] border border-line bg-white p-8 lg:grid-cols-[1fr_auto] lg:items-center lg:p-10">
              <div>
                ...
                <p className="mt-3 max-w-2xl text-ink/65 text-pretty">{master.blurb}</p>
                <p className="mt-4 font-mono text-[0.6rem] uppercase tracking-[0.16em] text-steel">
                  PDF · {fileSize(master.file)} · Updated {new Date().getFullYear()}
                </p>
              </div>
              <a
              ...
              >
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none" aria-hidden>
                  <path d="M6.5 1v8M3 6l3.5 3.5L10 6M1.5 11.5h10" stroke="currentColor" strokeWidth="1.5" />
                </svg>
                Download catalogue
              </a>
            </div>
            ...
            {ranges.map((r, i) => {
              ...
              return (
                <Reveal key={r.slug} delay={i * 70} className="h-full">
                  <article className="flex h-full flex-col rounded-[var(--radius-tile)] border border-line bg-white p-6 transition-colors hover:border-gold">
                    ...
                    <p className="mt-2.5 flex-1 text-[0.88rem] leading-relaxed text-ink/62">{r.tagline}</p>
                    ...
                    <div className="mt-5 flex flex-wrap items-center gap-3">
                      <a href={`/catalogue/${file}`} download className="btn btn-ink btn-sm">
                        Download PDF
                      </a>
                      <Link
          ...
          <Reveal className="mt-12 rounded-[var(--radius-tile)] border border-dashed border-line bg-ivory-2 p-7">
            ...
            <p className="mt-2 max-w-3xl text-[0.92rem] text-ink/65">
              Dimensional tables, weights per thousand and packing details are issued per item on
              request — tell us the range and the sizes you are buying and we will send the current
              sheet with your quotation. Write to{" "}
              <a href={`mailto:${site.contact.email}`} className="text-gold-ink underline">
                {site.contact.email}
              </a>
              .
            </p>
      ...
      <CTABand title="Quoting against a drawing?" body="Send the drawing with the catalogue reference and we will come back with grade, lead time and packing." />
    </>
  );
}
