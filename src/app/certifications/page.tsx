...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    description: "Exporter registrations held by us, and the manufacturing certifications held by our partners.",
    url: "/certifications",
  },
};
...
function Seal() {
  return (
    <svg viewBox="0 0 48 48" className="h-9 w-9 shrink-0 text-gold-ink" fill="none" aria-hidden>
      <path d="M24 4 42 12v14c0 10-8 15-18 18C14 41 6 36 6 26V12z" stroke="currentColor" strokeWidth="1.3" />
...
export default function CertificationsPage() {
  return (
    <>
      ...
      <section className="py-16 lg:py-24">
        <div className="shell grid gap-12 lg:grid-cols-2">
          <Reveal>
            ...
            <div className="mt-4 space-y-px overflow-hidden rounded-[var(--radius-tile)] border border-line bg-line">
              {ours.map((c) => (
                <article key={c.title} className="flex gap-4 bg-white p-6">
                  <Seal />
                  <div>
                    <h2 className="font-display text-[1.02rem] font-semibold tracking-tight">{c.title}</h2>
                    <p className="mt-1.5 text-[0.86rem] leading-relaxed text-ink/62">{c.body}</p>
                    <p className="mt-2.5 font-mono text-[0.76rem] text-gold-ink">{c.value}</p>
                  </div>
                </article>
              ))}
            </div>
          ...
          <Reveal delay={110}>
            ...
            <p className="mt-6 rounded-[var(--radius-tile)] border border-dashed border-line bg-ivory-2 p-6 text-[0.88rem] leading-relaxed text-ink/70">
              <strong className="font-semibold">Please note:</strong> {site.name} is a merchant
              exporter and does not operate manufacturing, forging or testing facilities. The
              certifications in this column belong to our sourcing partners and are listed as
              supplier certifications, not as our own manufacturing accreditation.
            </p>
          </Reveal>
        </div>
        ...
        <div className="shell mt-14">
          <Reveal>
            <div className="grid gap-6 rounded-[var(--radius-tile)] border border-line bg-white p-8 sm:grid-cols-3">
              {[
                ["Documents issued with every shipment", "Commercial invoice · Packing list · Certificate of origin · Mill test certificates · Bill of lading"],
                ["Payment terms", "Advance, LC at sight and negotiated terms for repeat buyers"],
                ["Incoterms", site.incoterms.join(" · ") + " · Loading at Kolkata, Haldia or Nhava Sheva"],
              ].map(([t, b]) => (
                <div key={t}>
                  <p className="spec-label">{t}</p>
                  <p className="mt-2 text-[0.9rem] leading-relaxed text-ink/72">{b}</p>
                </div>
              ))}
            </div>
          </Reveal>
      ...
      <CTABand title="Need our documents up front?" body="We can send the IEC and GST registration copies with the quotation — just ask when you enquire." />
    </>
  );
}
